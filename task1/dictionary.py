...
class Dictionary:
    ...
    def latin_words_proportion(self):
        latin_words = 0
        for k, v in self.dict.items():
            latin_words += v.cnt if all(ord(c) < 128 for c in k) else 0
        return latin_words / self.words_cnt

    def stop_words_proportion(self):
        ...
        stop_words_count = 0
        for k, v in self.dict.items():
            stop_words_count += v.cnt if k in stop_words else 0
